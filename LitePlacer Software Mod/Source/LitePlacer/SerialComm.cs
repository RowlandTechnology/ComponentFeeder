...
namespace LitePlacer
{
    class SerialComm
    {
       ...
       public SerialComm(CNC caller, FormMain MainF)
        ...
        public bool BinWrite(Byte[] data, int idx, int length)
        {
            try
            {
                if (!Port.IsOpen)
                ...
                if (Port.IsOpen)
                {
                    Port.Write(data, idx, length);
                }
                else
                {
                    MainForm.DisplayText("Serial port not open, write discarded.", KnownColor.DarkRed);
                }
                return true;
            }
            catch (Exception e)

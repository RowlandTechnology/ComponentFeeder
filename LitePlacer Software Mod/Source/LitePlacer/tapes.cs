...
namespace LitePlacer
{
	class TapesClass
	{
        ...
        public bool GotoNextPartByMeasurement_m(int TapeNumber, out double HoleX, out double HoleY)
		{
            ...
            while (!MainForm.GoToFeatureLocation_m(FormMain.FeatureType.Circle, 1.8, 0.5, out HoleX, out HoleY))
			{
				DialogResult x = MainForm.ShowMessageBox(
					"Can't find tape hole",
					"Tape error",
					MessageBoxButtons.AbortRetryIgnore
				);

                if (x == DialogResult.Abort)            //Bail Out
                {
                    return false;
                }
                else if (x == DialogResult.Ignore)      //Continue using existing coords
                {
                    break;  //Exit loop
                }
            }

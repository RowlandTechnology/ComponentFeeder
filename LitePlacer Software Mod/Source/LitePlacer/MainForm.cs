...
namespace LitePlacer
{
    ...
    public partial class FormMain : Form
    {
        ...
        //Rowland Technology - Interface Board Commands
        const byte CMD_SERVO_EN = 0x97;
        const byte CMD_SERVO_DIS = 0x98;
        const byte CMD_SERVO_POS = 0x99;
        //const byte CMD_SERVO_MID = 0x80;      //1-2ms Servo Motor
        const byte CMD_SERVO_MID = 0x55;        //1-2.5ms Servo Motor
        ...
        private void FormMain_Shown(object sender, EventArgs e)
        {
            ...
            Cnc.Connect(Setting.CNC_SerialPort, Setting.CNC_FeederPort);  // moved to here, as this can raise error condition, needing the form up
        ...
        */
        ...
        private void BasicSetupTab_Begin()
        {
            ...
            if (comboBoxSerialPortsFeeder.Items.Count == 0)
            {
                return;
            };
            ...
            found = false;
            i = 0;
            foreach (var item in comboBoxSerialPortsFeeder.Items)
            {
                if (item.ToString() == Setting.CNC_FeederPort)
                {
                    found = true;
                    comboBoxSerialPortsFeeder.SelectedIndex = i;
                    break;
                }
                i++;
            }
            ...
            {
                // show the first available port
                comboBoxSerialPortsFeeder.SelectedIndex = 0;
                return;
            }
        }
        ...
        private void RefreshPortList()
        {
            ...
            comboBoxSerialPortsFeeder.Items.Clear();
            foreach (string s in SerialPort.GetPortNames())
            {
                comboBoxSerialPorts.Items.Add(s);
                comboBoxSerialPortsFeeder.Items.Add(s);
            }
            if (comboBoxSerialPorts.Items.Count == 0)
            {
        ...
        public void UpdateCncConnectionStatus()
        {
            ...
            if (Cnc.ErrorState)
            {
                ...
                //Hopefully now fixed elsewhere, on now = off then on  
                //Added these otherwise they get left on and leads to a loss of vacuume next run
                //Pump_checkBox.Checked = false;
                //Vacuum_checkBox.Checked = false;
                //Cnc.PumpOff();
                //Cnc.VacuumOff();
        ...
        private void buttonConnectSerial_Click(object sender, EventArgs e)
        {
            ...
            if (Cnc.ErrorState || !Cnc.Connected)
            {
                ...
                if (Setting.CNC_FeederPort != comboBoxSerialPortsFeeder.SelectedItem.ToString())
                {
                    // user changed the selection
                    buttonConnectSerial.Text = "Closing..";
                    Cnc.Close();
                ...
                if (Cnc.Connect(comboBoxSerialPorts.SelectedItem.ToString(), comboBoxSerialPortsFeeder.SelectedItem.ToString()))
                {
                    buttonConnectSerial.Text = "Connecting..";
                    Cnc.ErrorState = false;
                    Setting.CNC_SerialPort = comboBoxSerialPorts.SelectedItem.ToString();
                    Setting.CNC_FeederPort = comboBoxSerialPortsFeeder.SelectedItem.ToString();
                    UpdateCncConnectionStatus();
                    if (ControlBoardJustConnected())
                    {
                        OfferHoming();
        ...
        */
        ...
        private void PlaceAll_button_Click(object sender, EventArgs e)
        {
            ...
            bool test = true;       //Default return home after finishing
            ...
            for (int i = 0; i < JobData_GridView.RowCount; i++)
            {
                ...
                if (!PlaceRow_m(i))
                {
                    test = false;       //Stop machine from returning to home
                    break;
                }
            }

            CleanupPlacement(test);
            if (test)
            {
                ShowMessageBox(
        ...
        private bool PlacePart_m(int CADdataRow, int JobDataRow, double X, double Y, double A, bool FirstInRow)
        {
            ...
            switch (Method)
            {
                ...
                case "Place Assisted":
                    ...
                    else
                    {
                        try
                        {
                            FeederId = Int32.Parse((string)Tapes_dataGridView.Rows[TapeNum].Cells["Feeder_Column"].Value);
                        }
            ...
            if (FeederId > 0)
            {
                if (Cnc.FeederConnected)
                {
                    Cnc.FeederWrite(new byte[] { (byte) ':', (byte) FeederId, (byte) 'A', (byte) '\n' }, 0, 4);   //Perform feeder movement
                }
            }

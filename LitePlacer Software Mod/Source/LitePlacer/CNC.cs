...
namespace LitePlacer
{
    ...
    public class CNC
    {
        ...
        public bool FeederConnected { get; set; }
        ...
        // =================================================================================
        public bool Connect(String name, String feederName)
        {
            // For now, just see that the port opens. 
            // TODO: check that there isTinyG, not just any comm port.
            // TODO: check/set default values

            if (string.Compare(name, feederName) != 0 )     //Check Feeder and TinyG ports don't match. If they do then don't open Feeder port
            {
                if (ComFeeder.IsOpen)
                {
                    MainForm.DisplayText("Already connected to feeder, port " + feederName + ": already open");
                ...
                Homing = false;
                _readyEvent.Set();
                FeederConnected = ComFeeder.IsOpen;
                if (!FeederConnected)
                {
                    MainForm.DisplayText("Connecting to feeder port " + feederName + " failed.");
                    Error();
                }
                else
                {
                    MainForm.DisplayText("Connected to feeder port " + feederName);
                }
            }
            else
            {
                MainForm.DisplayText("Feeder port not connected - same port as TinyG." + name);
            }
        ...
        public bool FeederWrite(Byte[] data, int idx, int length)
        {
            bool res = false;

            if (ComFeeder.IsOpen)
            {
                res = ComFeeder.BinWrite(data, idx, length);
            }
